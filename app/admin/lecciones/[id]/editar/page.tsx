...
export default async function EditLessonPage({
  ...
  const { data: lesson } = await supabase
    .from('lessons')
    .select(`
      id, module_id, title, lesson_type, content,
      cover_image_url, cover_storage_bucket, cover_storage_path, position,
      lesson_assets ( id, asset_type, title, file_url, position, provider, provider_file_id )
    `)
    .eq('id', id)
    .single()
