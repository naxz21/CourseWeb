...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (lastModuleError) return NextResponse.json({ error: lastModuleError.message }, { status: 400 })

    const nextPosition = (lastModule?.position || 0) + 1

    const result = await supabaseAdmin.from('modules').insert({ course_id: courseId, title, description, position: nextPosition })
